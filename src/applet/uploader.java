...
public final class uploader{
	public static void main(final String[]a)throws Throwable{
		if(a.length<5){
			System.out.println("i.e. java "+upload.class.getName()+" localhost 8888 aaaa-110307-064915.110-cb48e04f upload file.txt q");
			return;
		}
		final boolean silent=a.length>5&&"q".equals(a[5]);
		final uploader upl=new uploader();
		upl.perfstart();
		upl.send(a[0],Integer.parseInt(a[1]),a[2],a[3],new File(a[4]),null,silent?new stsb():new stsbarcon());
		upl.perfstop();
	...
	public synchronized void send(final String host,final int port,final String session,final String dir,final File fl,final String md5sum,final stsb pb)throws IOException{
		...
		while(true){
			...
			n+=count;
			totalbytes+=count;
			pb.sts(totalbytes+"   "+dcf.format(n*100/filelen)+"%  "+fl.getName()+"    "+dcf.format(n)+"/"+dcf.format(filelen)+" bytes");
		}
		is.close();
	}
}